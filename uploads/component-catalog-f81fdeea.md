...
| Key | Label | Type | Default | Options / Notes |
|---|---|---|---|---|
| `eyebrow` | Eyebrow Line | text | `''` | Optional small label rendered above the title with `class="eyebrow"`. Style with a `.eyebrow { ... }` rule in your `styles.css` (uppercase + tracked + accent colour is the typical treatment). |
| `title` | Title | text | `Welcome` | Plain text — HTML is escaped. Use `titleAccent` for a second-voice phrase. |
| `titleAccent` | Title Accent | text | `''` | Optional secondary phrase rendered as `<em>` inside the same `<h1>`. Style with `h1 em { font-family: var(--tenant-font-accent); color: ...; }` to give it a different font / colour from the main title (script accents, italic emphasis, etc.). |
| `subtitle` | Subtitle | text | `Your tagline here` | Rendered as a paragraph below the title. Set to `""` if you're using `eyebrow` instead. |
| `description` | Description | textarea | `''` | |
...
| `heroHeight` | Hero Height | select | `large` | `medium`, `large`, `full` |
| `showLogo` | Show Logo In Frame | bool | `true` | Set `false` when the navigation already shows the logo and you don't want it repeated inside the hero panel. |
