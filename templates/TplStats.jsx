...
const TplStats = ({
  ...
  return (
    <div style={{
      ...
      {/* stats row */}
      <div style={{ marginTop: 32*s, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12*s }}>
        {stats.map((st, i) => (
          <Field name={`stat_${i+1}`} key={i}>
            <div style={{
              background: tw.card, borderRadius: 8,
              padding: `${22*s}px ${16*s}px`,
              textAlign: 'center',
            }}>
              <div style={{
                fontFamily: tw.display, fontWeight: 900,
                fontSize: 80*s, color: tw.brick, lineHeight: 1,
                letterSpacing: -2, marginBottom: 12*s,
              }}>{st.value}</div>
              <div style={{
                fontFamily: tw.mono, fontSize: 12*s, lineHeight: 1.5,
                color: tw.inkMuted,
              }}>{st.body}</div>
            </div>
          </Field>
